...
const fmt = (n: any) => (n === "" || n === null || n === undefined ? "—" : n);
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      const existing = data.find((s: any) => s.date === todayStr);
  ...
  const save = (updatedToday: any, updatedSessions: any) => {
    const todayStr = new Date().toISOString().split("T")[0];
  ...
  const updateToday = (section: string, field: string, value: any) => {
    const updated = {
      ...today,
...
function TodayView({ today, updateToday, advancePhase }: any) {
  ...
  return (
    <div style={styles.todayWrap}>
        ...
        >
          <div style={styles.grid2}>
            <Field
              ...
              onChange={(v: any) => updateToday("commitment", "leadsToFind", v)}
              type="number"
              placeholder="e.g. 20"
              locked={phase === "complete"}
            />
            <Field
              ...
              onChange={(v: any) => updateToday("commitment", "leadsToQualify", v)}
              type="number"
              placeholder="e.g. 10"
              locked={phase === "complete"}
            />
            ...
            <Field
              ...
              onChange={(v: any) => updateToday("commitment", "emailsToSend", v)}
              type="number"
              placeholder="e.g. 5"
              locked={phase === "complete"}
            />
          ...
          <Field
            ...
            onChange={(v: any) => updateToday("commitment", "nicheFocus", v)}
          ...
          <CheckField
            ...
            onChange={(v: any) => updateToday("commitment", "stateActivated", v)}
            locked={phase === "complete"}
            inline={false}
          />
          {phase === "pre" && (
        ...
        >
          <div style={styles.grid2}>
            <Field
              ...
              onChange={(v: any) => updateToday("actuals", "leadsFound", v)}
              type="number"
              placeholder="actual"
              locked={phase === "complete"}
            />
            <Field
              ...
              onChange={(v: any) => updateToday("actuals", "leadsQualified", v)}
              type="number"
              placeholder="actual"
              locked={phase === "complete"}
            />
            <Field
              ...
              onChange={(v: any) => updateToday("actuals", "scriptsGenerated", v)}
              type="number"
              placeholder="actual"
              locked={phase === "complete"}
            />
            <Field
              ...
              onChange={(v: any) => updateToday("actuals", "emailsSent", v)}
              type="number"
              placeholder="actual"
              locked={phase === "complete"}
            />
          ...
          <div style={styles.grid2}>
            <Field
              ...
              onChange={(v: any) => updateToday("actuals", "drifts", v)}
              type="number"
              placeholder="count"
              locked={phase === "complete"}
            />
          ...
          <div style={styles.checkRow}>
            <CheckField
              ...
              onChange={(v: any) => updateToday("actuals", "resistanceHit", v)}
              locked={phase === "complete"}
              inline
            />
            <CheckField
              ...
              onChange={(v: any) => updateToday("actuals", "resistanceHandled", v)}
              locked={phase === "complete"}
              inline
            />
          </div>
      ...
      {(phase === "post" || phase === "complete") && (
        ...
        >
          <Field
            ...
            onChange={(v: any) => updateToday("review", "biggestWin", v)}
          ...
          <Field
            ...
            onChange={(v: any) => updateToday("review", "mainObstacle", v)}
          ...
          <Field
            ...
            onChange={(v: any) => updateToday("review", "nextIntent", v)}
      ...
      {phase === "complete" && (
        <Section label="04 — SESSION SUMMARY" sublabel="Today's numbers" locked={false}>
          <SummaryRow
...
function SummaryRow({ commitment, actuals }: any) {
  const rows = [
...
function LogView({ sessions, save, setSessions }: any) {
  const [editing, setEditing] = useState<number | null>(null);
  const [emailData, setEmailData] = useState<any>({});

  const handleEmailUpdate = (sessionId: number, field: string, value: any) => {
  ...
  const saveEmailMetrics = (session: any) => {
    const metrics = emailData[session.id] || {};
    const updated = { ...session, emailMetrics: { ...session.emailMetrics, ...metrics } };
    const newSessions = sessions.map((s: any) => s.id === session.id ? updated : s);
    setSessions(newSessions);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(newSessions));
    setEditing(null);
  };

  const completed = sessions.filter((s: any) => s.phase === "complete");
        ...
        <div key={s.id} style={styles.logCard}>
          <div style={styles.logCardTop} onClick={() => setEditing(editing === s.id ? null : s.id)}>
            ...
            <div style={styles.logMetrics}>
              <Chip label="Found" value={fmt(s.actuals.leadsFound)} accent={false} />
              <Chip label="Qual" value={fmt(s.actuals.leadsQualified)} accent={false} />
              <Chip label="Scripts" value={fmt(s.actuals.scriptsGenerated)} accent={false} />
              <Chip label="Sent" value={fmt(s.actuals.emailsSent)} accent={false} />
...
function MetricsView({ sessions }: any) {
  const completed = sessions.filter((s: any) => s.phase === "complete");
  ...
  const sum = (arr: any[], fn: any) => arr.reduce((acc, s) => acc + (parseInt(fn(s)) || 0), 0);

  const totalFound = sum(completed, (s: any) => s.actuals.leadsFound);
  const totalQual = sum(completed, (s: any) => s.actuals.leadsQualified);
  const totalScripts = sum(completed, (s: any) => s.actuals.scriptsGenerated);
  const totalSent = sum(completed, (s: any) => s.actuals.emailsSent);
  const totalOpens = sum(completed, (s: any) => s.emailMetrics?.opens);
  const totalClicks = sum(completed, (s: any) => s.emailMetrics?.clicks);
  const totalReplies = sum(completed, (s: any) => s.emailMetrics?.replies);

  const avg = (fn: any) => {
    const vals = completed.map((s: any) => parseInt(fn(s)) || 0).filter((v: number) => v > 0);
    return vals.length ? Math.round(vals.reduce((a: number, b: number) => a + b) / vals.length) : "—";
  };
  ...
  const flowSessions = completed.filter(
    (s: any) => s.actuals.alignment === 3 && (parseInt(s.actuals.drifts) || 99) <= 5
  ).length;
  ...
  return (
    <div style={styles.metricsWrap}>
      ...
      <div style={styles.kpiRow}>
        ...
        <KPI label="Avg emails/session" value={avg((s: any) => s.actuals.emailsSent)} sub="actual" />
      </div>

      <div style={styles.funnelSection}>
        <span style={styles.funnelTitle}>CUMULATIVE FUNNEL</span>
        <FunnelBar label="Leads found" value={totalFound} max={totalFound} accent={false} />
        <FunnelBar label="Leads qualified" value={totalQual} max={totalFound} accent={false} />
        <FunnelBar label="Scripts generated" value={totalScripts} max={totalFound} accent={false} />
        <FunnelBar label="Emails sent" value={totalSent} max={totalFound} accent={false} />
      ...
      {completed.length >= 3 && (
        <div style={styles.trendSection}>
          ...
          <SparkLine
            data={completed.slice(-10).map((s: any) => parseInt(s.actuals.emailsSent) || 0)}
          />
        </div>
      )}
    </div>
...
function Section({ label, sublabel, locked, children }: any) {
  return (
...
function Field({ label, value, onChange, type = "text", placeholder, locked }: any) {
  return (
...
function CheckField({ label, checked, onChange, locked, inline }: any) {
  return (
    <div
...
function Chip({ label, value, accent }: any) {
  return (
...
function FunnelBar({ label, value, max, accent }: any) {
...
function ConvCell({ label, value }: any) {
  return (
